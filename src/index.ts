...
app.delete("/accounts/:id", (req: Request, res: Response) => {
    ...
    try {
        const id = req.params.id
           ...
           if(id[0] !== "a"){
               res.status(400)
               throw new Error ("'id' inválido. Deve ser iniciado com a letra 'a'.")
           }
    ...
    try {
        ...
        if(newBalance !== undefined){
            ...
            if(typeof newBalance !== "number"){
                res.status(400)
                throw new Error("'Balance' dever ser um número");
            }

            if(newBalance<0){
                res.status(400)
                throw new Error("'Balance' dever ser maior que zero");
            }
        }
        ...
        if(newType !== undefined){
            ...
            if(
                ...
                newType !== ACCOUNT_TYPE.BLACK ){
                    res.status(400)
                    throw new Error("'type' deve ser igual a Gold, platinum ou black")
                }
        }
